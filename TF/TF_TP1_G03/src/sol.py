...
response_queue = {}  # contains queue de "mini events" | key (request id) - value (a resposta)
...
while True:
    ...
    # Reads from a quorum and returns the updated value
        ...
        for node in nodes:
            sendSimple(node_id, node, type=QR_READ, request_id=request_id, key=key)
    ...
    # Returns the key's timestamp or an error in case the key doesn't exist
        ...
        for node in nodes:
            sendSimple(node_id,node,type=QR_WRITE, request_id=request_id, key=key, value=value)
